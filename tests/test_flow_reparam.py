...
def test_per_param_qualification():
    """A dist param earns a per-element knob iff it depends on another
    latent; a param that is constant (or data-only) gets a size-0 knob,
    pinned at the centred no-op — regardless of how it maps onto the RV's
    axes (broadcast hyperprior or 1:1 full-shape)."""
    ...
    # Full-shape (1:1 elementwise) scale that still depends on a latent —
    # each y_i funnels off its own sigma_i — earns a per-element knob.
    with pm.Model(coords=coords) as m:
        mu = pm.Normal("mu", 0, 1)
...
@pytest.mark.pymc
def test_scalar_funnel_reparametrized():
    """Neal's funnel: a scalar latent whose scale funnels off another
    scalar latent fires, with the knob itself a scalar (shape ()). The
    dependence is 1:1 with no broadcast axis, and the value is 0-d — both
    used to be skipped."""
    with pm.Model() as m:
        z = pm.Normal("z", 0.0, 3.0)
        pm.Normal("x", 0.0, pt.exp(z / 2))

    compiled, records, constrain_fn, unconstrain_fn = _compile_flow(m)
    assert records["z"]["flow_cls"] is NoFlow
    assert records["x"]["flow_cls"] is AffineFlow
    # loc = 0 withheld; the scale knob is a scalar.
    assert records["x"]["param_shapes"] == [(0,), ()]

    n_dim = int(compiled.n_dim)
    assert n_dim == 2
    z_sl = _var_slice(compiled, "z")
    x_sl = _var_slice(compiled, "x")

    rng = np.random.default_rng(0)
    y = rng.normal(size=n_dim)
    h_sigma = 0.6
    flow_params = np.array([h_sigma])

    value, ljd_c = constrain_fn(y, flow_params)
    sigma = np.exp(y[z_sl][0] / 2)
    np.testing.assert_allclose(value[x_sl], sigma**h_sigma * y[x_sl], atol=1e-10)
    np.testing.assert_allclose(ljd_c, h_sigma * np.log(sigma), atol=1e-10)
...
@pytest.mark.pymc
def test_imputation_observed_rv_no_rng_leak():
    # Auto-imputation (observed array with missing entries) introduces observed
    # RVs that carry an RNG. They must resolve to their observed data in the
    # flow graph (as PyMC's logp does), leaving no RandomVariable/RNG behind —
    # otherwise build_auto_flow's jit_fn gains spurious
    # random_generator_shared_variable inputs and sampling fails.
    from pytensor.graph.basic import ancestors
    from pytensor.tensor.random.op import RandomVariable

    rng = np.random.default_rng(0)
    data = rng.normal(size=12)
    data[::3] = np.nan  # missing entries -> PyMC auto-imputation
    with pm.Model() as m:
        mu = pm.Normal("mu", 0, 1)
        sigma = pm.HalfNormal("sigma", 1)
        pm.Normal("y", mu, sigma, observed=np.ma.masked_invalid(data))

    records = automatic_flow_reparam(m)
    (imputed,) = [n for n in records if "unobserved" in n]
    assert records[imputed]["flow_cls"] is AffineFlow

    compiled = nutpie.compile_pymc_model(m, backend="jax", gradient_backend="jax")
    g = build_flow_graph(m, free_vars_info(compiled), compiled.n_dim)
    leftover = [
        a
        for a in ancestors(g["constrain"][1])
        if a.owner is not None and isinstance(a.owner.op, RandomVariable)
    ]
    assert leftover == [], f"RNG-carrying RVs leaked into flow graph: {leftover}"

    # The jit_fn path (build_auto_flow) must compile without spurious RNG args.
    assert build_auto_flow(m, compiled) is not None

    constrain_fn = pytensor.function(*g["constrain"])
    unconstrain_fn = pytensor.function(*g["unconstrain"])
    n_dim = int(compiled.n_dim)
    npar = int(g["flow_params_vector"].type.shape[0])
    y0 = rng.normal(size=n_dim)
    params = rng.normal(size=npar) * 0.3
    value, _ = constrain_fn(y0, params)
    back, _ = unconstrain_fn(value, params)
    np.testing.assert_allclose(back, y0, atol=1e-9)
