...
def _stack_shape(dims):
    """Stack scalar shape components into a 1-D int64 shape vector,
    returning the empty vector (shape ``()``) when there are none — plain
    ``pt.stack([])`` raises ``"No tensor arguments provided."``."""
    if not dims:
        return pt.constant(np.array([], dtype="int64"))
    return pt.stack(dims)
...
def _hyper_shape(value, n_core: int = 0):
    ...
    ndim = value.ndim
    dims = [*(value.shape[i] for i in range(ndim - n_core)), *([_one] * n_core)]
    return _stack_shape(dims)
...
def loc_scale_affine_flow(fgraph, node):
    ...
    # A param earns a centering knob when it carries free-RV randomness —
    # i.e. the modeller left the RV (fully or partly) centred on another
    # latent, so there is prior dependence to decouple. Where centring is
    # already best the fitted knob collapses to the no-op (h = 0).
    loc_qualifies = _depends_on_free_rv([loc])
    scale_qualifies = _depends_on_free_rv([scale])
...
def _first_non_model_var(var):
    while var.owner is not None and isinstance(var.owner.op, ModelVar):
        op = var.owner.op
        # Observed RVs (valued, but not free/flow) resolve to their observed
        # value (data, inputs[1]); using the rv (inputs[0]) would leak its
        # RNG into the flow graph (PyMC's logp likewise substitutes observed
        # RVs by their data). Free/flow RVs and other ModelVars use inputs[0].
        if isinstance(op, ModelValuedVar) and not isinstance(
            op, (ModelFreeRV, FlowFreeRV)
        ):
            var = var.owner.inputs[1]
        else:
...
def reparametrize(
    ...
    for node in fgraph.toposort():
        ...
        if not isinstance(op, ModelVar):
            continue
        # Observed RVs strip to their observed value (data, inputs[1]); the rv
        # (inputs[0]) carries an RNG that would leak into the flow graph. Free
        # RVs become their rv (replaced by value vars later); other ModelVars
        # (deterministics, …) use inputs[0].
        if isinstance(op, ModelValuedVar) and not isinstance(
            op, (ModelFreeRV, FlowFreeRV)
        ):
            repl = _first_non_model_var(node.inputs[1])
        else:
            repl = _first_non_model_var(node.inputs[0])
        dummy_replacements.append((node.outputs[0], repl))
...
def build_auto_flow(
    ...
    *,
    ...
    centering knobs ``h`` (the paper's ``1 - λ``); ``init_params`` defaults
    to zeros, i.e. ``h = 0`` — the centred (CP) no-op, so adaptation starts
    from the original model and moves toward non-centring as needed (see
    :class:`~nutpie.flow_reparam.AffineFlow`).
