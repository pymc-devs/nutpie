...
download = pyjs.js.Function('url', 'path', '''return (async () => {
...
summary = {'python': sys.version, 'nutpie': nutpie.__version__, 'extension': nutpie._lib.__file__}
...
def verify_density(model, compiled):
    ...
    for seed in [11, 12, 13]:
        ...
        np.testing.assert_allclose(gradient, reference_grad(point), rtol=1e-8, atol=1e-8)
...
assert abs(values.mean()) < .2 and abs(values.std()-1) < .2
assert all(abs(row['twice'][0]-2*row['mu'][0]) < 1e-12 for chain in result for row in chain)
assert nutpie.sample_raw(native, draws=20, tune=30, chains=1, seed=17) == nutpie.sample_raw(native, draws=20, tune=30, chains=1, seed=17)
summary['gaussian'] = {'shape': list(values.shape), 'mean': float(values.mean()), 'std': float(values.std()), 'seconds': time.perf_counter()-started}
...
for row in result[0]:
    ...
    np.testing.assert_allclose(row['scaled'], np.asarray(row['simplex'])*row['scale'][0], atol=1e-12)
...
with pm.Model() as data_model:
    data = pm.Data('data', np.array([1.,2.,3.], dtype='float32'))
...
updated = data_compiled.with_data(data=np.array([4.,5.,6.],dtype='float32'))
updated_native = updated._make_model(None)
a = nutpie._lib.evaluate_pymc(data_native, [0.])
b = nutpie._lib.evaluate_pymc(updated_native, [0.])
np.testing.assert_allclose(a[1], [5.])
np.testing.assert_allclose(b[1], [14.])
assert a != b
assert nutpie._lib.evaluate_pymc(data_native, [0.]) == a
# Native model retains callback owners after Python compiled handles are dropped.
del data_compiled, updated
gc.collect()
assert nutpie._lib.evaluate_pymc(data_native, [0.]) == a
assert nutpie._lib.evaluate_pymc(updated_native, [0.]) == b
...
assert nutpie._lib.evaluate_pymc(updated_native, [0.]) == b
summary['mutable_data'] = {'old_gradient': a[1], 'new_gradient': b[1], 'ownership_and_memory_growth': True}
...
summary['mmm']={'dimensions':int(mmm_compiled.n_dim), 'draws':50, 'tune':100, 'density_gradient_checks':3, 'seconds':time.perf_counter()-started}
